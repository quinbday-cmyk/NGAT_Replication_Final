...
class Model_G(torch.nn.Module):
    def __init__(self, in_feature:int, out_feature:int, num_nodes:int, edge_dim:int, matrix_in:bool, batch_size:int, classification:bool):
        ...
        self.lstm_for_index = LSTM_layer(input_size=self.in_feature, hidden_size=self.hidden, num_layers=1, batch_size=1, batch_first=True)

        self.output = nn.Linear(2 * self.hidden, self.out_feature)
        if self.classification:
            self.sigmoid = nn.Sigmoid()

    def forward(self, data, index_x):
        ...
        x = F.elu(x)
        ngat_x = x.mean(dim=0, keepdim=True)
        
        index_x = index_x.reshape(1, -1, self.in_feature)
        lstm_x = self.lstm_for_index(index_x)
        lstm_x = F.dropout(lstm_x, self.dropout, training=self.training)

        x = torch.concatenate([ngat_x, lstm_x], dim=-1)
