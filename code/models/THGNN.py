...
class THGNN(nn.Module):
    def __init__(self, in_features=6, out_features=8, num_heads=8, hidden_dim=64, num_layers=1, device='cuda'):
    ...
    def forward(self, data, requires_weight=False):
        inputs, edge_index, edge_attr = data.x, data.edge_index, data.edge_attr
        inputs = inputs.double()
        print(inputs.dtype) 
        N = inputs.shape[0]       # num_nodes
        pos_adj = torch.zeros([N, N], device=self.device, dtype=torch.float64)
        neg_adj = torch.zeros([N, N], device=self.device, dtype=torch.float64)

        # Update adjacency matrices based on edge_index and edge_attr
        for i in range(edge_index.shape[1]):
            source, target = edge_index[:, i]
            if edge_attr[i] > 0:
                pos_adj[source, target] = 1
            else:
                neg_adj[source, target] = 1
        
        # # Update adjacency matrices based on edge_index and edge_attr without loop
        # pos_mask = edge_attr > 0
        # neg_mask = edge_attr <= 0
        # pos_adj[edge_index[0, pos_mask], edge_index[1, pos_mask]] = 1
        # neg_adj[edge_index[0, neg_mask], edge_index[1, neg_mask]] = 1
